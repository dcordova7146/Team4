...
const BULLET: PackedScene = preload("res://projectile/machine_gun_bullet.tscn")
## How long until the gun can be fired again.
@export var cooldown_duration: float = 0.1  # Reduce cooldown for rapid firing
## How long until the game is fired again when primary action key held.
@export var auto_fire_duration: float = 0.01  # Rapid firing when primary action held
